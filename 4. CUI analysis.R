...
for (PK_metric in c("Cmax", "AUC", "Cmin")) { # Loop through the pk-metrics
  PK_metricO <- PK_metric # Original metric for saving results
  ...
  for (ADC in c("TDM1", "TDXd")) { # Loop through the ADCs
    ...
    PK_metric <- PK_metricO
    if (ADC == "TDM1" & PK_metric == "Cmin") PK_metric <- "Cmax" # As a placeholder to facilitate analysis (ER analysis for TDM1 and Cmin not possible)
    ...
    for (cui_improvement in cui_improvements) { 
      for (i in seq_along(scenarios)) {
        ...
        for (j in seq_along(results_tb$Covs)) {
          ...
          message(paste0(ADC, ", ", PK_metricO, ", CUI threshold ", cui_improvement, ": ", round(j * 100/length(results_tb$Covs), 3), "% complete!"))
  ...
  write_rds(results_doses, paste0("results/", PK_metricO, "_cui_thresholds.rds")) 
}
...
for (PK_metric in c("Cmax", "AUC", "Cmin")) { # Loop through the pk-metrics
  ...
  for (ADC in c("TDM1", "TDXd")) { # Loop through the ADCs
    ...
    font_size <- 10
    bumpplot_tdm1 <- read_rds(paste0("results/", PK_metric, "_cui_thresholds.rds")) %>%
      rename(Dose = dose) %>%
      filter(ADC == "TDM1") %>% 
    ...
    bumpplot_tdxd <- read_rds(paste0("results/", PK_metric, "_cui_thresholds.rds")) %>%
      rename(Dose = dose) %>%
      filter(ADC == "TDXd") %>% 
    ...
    # Save plot
    x <- 0.8
    graph2ppt(combined_plot, paste0("results/", PK_metricO, "_CUI_thresholds"), append = FALSE, width = 12 * x, height = 9 * x)
    ...
    for (cui_improvement in cui_improvements) { 
      for (i in seq_along(scenarios)) {
        ...
        for (j in seq_along(results_tb$Covs)) {
          ...
          message(paste0(ADC, ", ", PK_metricO, ": ", round(j * 100/length(results_tb$Covs), 3), "% complete!"))
        }
...
"
      ...
      graph2ppt(combined_plot, paste0("results/", ADC, "_", PK_metricO, "_", gsub("\\.", "_", as.character(cui_improvement)), "_CUI"), append = FALSE, width = 16 * x, height = 12 * x)
    }
  }
}
